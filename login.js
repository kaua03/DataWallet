...
function renderMascotEmotions() {
    ...
    if (isErrorMode) {
        if (estadoMascote !== 'triste') { animarBoca('triste'); estadoMascote = 'triste'; }
        return;
    }
...
function rastrearOlhar(clientX, clientY) {
    ...
    const isHoveringMascot = clientX >= rect.left && clientX <= rect.right && clientY >= rect.top && clientY <= rect.bottom;
...
[identInput, usernameInput].forEach(el => {
    ...
    el.addEventListener('focus', () => {
        if (!isErrorMode) { eyelidL.style.transform = 'scaleY(0)'; eyelidR.style.transform = 'scaleY(0)'; trackTyping(el); }
    });
});
...
passInput.addEventListener('focus', () => {
    if (!isErrorMode) {
        if (passInput.type === 'password') { eyelidL.style.transform = 'scaleY(0.6)'; eyelidR.style.transform = 'scaleY(0.6)'; } 
        else { eyelidL.style.transform = 'scaleY(0)'; eyelidR.style.transform = 'scaleY(0)'; }
...
passInput.addEventListener('blur', () => {
    if (!isErrorMode) {
        eyelidL.style.transform = 'scaleY(0)'; eyelidR.style.transform = 'scaleY(0)';
        pupils.forEach(p => p.style.transform = `translate(0px, 0px)`);
        barLeft.style.height = '64px'; barRight.style.height = '80px';
    }
});
...
togglePassBtn.addEventListener('click', () => {
    ...
    if (passInput.type === 'password') {
        passInput.type = 'text'; iconPass.classList.replace('fa-eye', 'fa-eye-slash');
        eyelidL.style.transform = 'scaleY(0)'; eyelidR.style.transform = 'scaleY(0)'; 
        trackTyping(passInput); 
    } else {
        passInput.type = 'password'; iconPass.classList.replace('fa-eye-slash', 'fa-eye');
        eyelidL.style.transform = 'scaleY(0.6)'; eyelidR.style.transform = 'scaleY(0.6)';
        passInput.focus(); trackTyping(passInput); 
    }
});
...
setInterval(() => {
    ...
    eyelidL.style.transform = 'scaleY(1)'; eyelidR.style.transform = 'scaleY(1)';
    setTimeout(() => {
...
// ==========================================
// MODO TELA E SISTEMA DE ERROS AUTODIAGNÓSTICO
// ==========================================
...
function dispararErroVisual() {
    isErrorMode = true; 
    const containers = [identInput.closest('.group'), passInput.closest('.group'), usernameInput.closest('.group')];
    ...
    try {
        ...
        if (isLogin) {
            ...
            if (!identificador.includes('@')) {
                ...
                if (errUser) {
                    console.warn("Aviso de Banco:", errUser);
